...
if user_input_1 in BEER_df['Name of Beer'].values:
    ...
    # Print value list
        brewer = BEER_df.loc[BEER_df['Name of Beer'] == user_input_1, 'Brewery'].values[0]
...
user_input_2 = st.text_input('Enter Another Name of a Beer From the Table To Compare to the First Beer:', 'Beer Name')
...
if user_input_2 in BEER_df['Name of Beer'].values:
    ...
    if not beer_row.empty:
        ...
        brewer = BEER_df.loc[BEER_df['Name of Beer'] == user_input_2, 'Brewery'].values[0]
...
fig = px.imshow(BEER_Standard_Style_heatmap.set_index('Brewing Style'),
                ...
                title="Beer Style Attribute Heatmap",
                width=1000,
                height=2000,
                zmin = -3,
                zmax = 3,
...
fig.update_layout(
    coloraxis_colorbar=dict(
        thickness=15,  # Change thickness
        titlefont=dict(size=12),  # Title font size
        tickfont=dict(size=10),  # Tick font size
        len=0.5,  # Length of the color bar
        x=1.0,  # X position (1 is the right edge)
        y=0.5,  # Y position (0 is the bottom, 1 is the top)
    )
)
st.plotly_chart(fig)
...
# Attribute Corr heatmap
# create a corr matrix
merged_flavor_rating_corr = BEER_df[['ABV', 'Ave Rating', 'Min IBU', 'Max IBU', 'Style Color Mean (SRM)', 'Astringency', 'Body', 'Alcohol', 'Bitter', 'Sweet', 'Sour', 'Salty', 'Fruits', 'Hoppy', 'Spices', 'Malty']]
correlation_matrix = merged_flavor_rating_corr.corr()
# lower triangle matrix (Not working as intended but keeping incase I figure it out.)
# mask = np.triu(np.ones_like(correlation_matrix, dtype=bool))
# masked_corr_matrix = np.where(mask, np.nan, correlation_matrix)

fig = px.imshow(correlation_matrix,
                # mask = mask,
                color_continuous_scale='YlGnBu',
                labels=dict(x= 'Attributes', y='Attributes'),
                title="Beer Attribute Correlation Heatmap",
                width=800,
                height=700,
                zmin = -0.5,
                zmax = 1,
